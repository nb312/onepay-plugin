...
/**
 * OnePay Visa Payment Gateway
 * Visa信用卡支付网关
 */
class WC_Gateway_OnePay_Visa extends WC_Payment_Gateway {
    
    protected $parent_gateway;
    
    public function __construct() {
        $this->id                 = 'onepay_visa';
        $this->icon               = ONEPAY_PLUGIN_URL . 'assets/images/cards/visa.svg';
        $this->has_fields         = true;
        $this->method_title       = __('Visa', 'onepay');
        $this->method_description = __('Visa信用卡支付', 'onepay');
    ...
    public function init_form_fields() {
        $this->form_fields = array(
            'enabled' => array(
                ...
                'type'    => 'checkbox',
                'label'   => __('启用Visa支付', 'onepay'),
                'default' => 'yes'
            ),
            'title' => array(
                'title'       => __('标题', 'onepay'),
                'type'        => 'text',
                'description' => __('客户在结账时看到的标题（固定为VISA）', 'onepay'),
                'default'     => __('VISA', 'onepay'),
    ...
    public function payment_scripts() {
        if (!is_admin() && (is_cart() || is_checkout() || isset($_GET['pay_for_order']))) {
            wp_enqueue_script('onepay-visa', ONEPAY_PLUGIN_URL . 'assets/js/onepay-visa.js', array('jquery'), ONEPAY_VERSION, true);
            wp_enqueue_style('onepay-visa', ONEPAY_PLUGIN_URL . 'assets/css/onepay-visa.css', array(), ONEPAY_VERSION);
        }
    }
    ...
    public function validate_fields() {
        ...
        if (!preg_match('/^4[0-9]{12}(?:[0-9]{3})?$/', $card_number)) {
            wc_add_notice(__('请输入有效的Visa卡号', 'onepay'), 'error');
            return false;
        }
    ...
    /**
     * 处理管理员选项保存，确保标题始终为VISA
     */
    public function process_admin_options() {
        $saved = parent::process_admin_options();
        
        // 强制设置标题为VISA，防止被管理员修改
        $this->update_option('title', 'VISA');
        $this->title = 'VISA';
